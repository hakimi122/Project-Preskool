                        ...
                        <div class="row align-items-center">
                            ...
                            <c:if test="${sessionScope.user.getRole() == 'headmaster'}">
                                <div class="col-auto text-right float-right ml-auto">
                                    <a href="${contextPath}/subject?action=add" class="btn btn-primary"><i class="fas fa-plus"></i></a>
                                </div>
                            </c:if>
                        </div>
                    </div>
                                            ...
                                            <tbody>
                                                <c:if test="${subjects != null}">
                                                    <c:forEach items="${subjects}" var="t">
                                                        <tr >
                                                            <td>${t.getSubjectId()}</td>
                                                            <td>${t.getSubjectName()}</td>
                                                            <td>${t.isTeaching}</td>
                                                            <c:if test="${sessionScope.user.getRole() == 'headmaster'}">
                                                                <td class="text-right">
                                                                    <div class="actions">
                                                                        <a href="${contextPath}/subject?action=edit&subjectId=${t.getSubjectId()}" class="btn btn-sm bg-success-light mr-2 editBtn">
                                                                            <i class="fas fa-pen"></i>
                                                                        </a>
                                                                        <a href="#" class="btn btn-sm bg-danger-light deleteBtn" data-subjectid="${t.getSubjectId()}">
            ...
            $(document).ready(function () {
                $(document).on('click', '.deleteBtn', function (e) {
                    ...
                    if (confirm("Are you sure you want to delete this item?")) {
                        ...
                        window.location.href = "${contextPath}/subject?action=delete&subjectId=" + userId;
                    } else {
